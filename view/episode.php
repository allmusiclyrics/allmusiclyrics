...
echo 'href="?show='.str_replace ($search,'-', $getShow['showname']).'&showid='.$getEpisode['showid'].'">'. $getShow['showname'].'</a> ';
echo $seasonepisode2.' '.$long.$total.' ('.$short.') songs list sound track called "'.$getEpisode['title'].'"<br>';
if ($getEpisode['date']){
...
if($getEpisode['magnet'])
	echo '<br><br><a href="'.$getEpisode['magnet'].'">Magnet</a> (<a href="?p=magnet" title="find out more about magnet links">?</a>)';
echo '<br>';
if($_SESSION['user']['department']=='admins'){
	...
	'" placeholder=text >';
	//echo '<input type=button value="Save tunefind" onclick="'.$action.'">';
	...
	//echo '<a href="'.$scrape.'http://www.tunefind.com/show/'.$nametunefind.'/season-'.$getEpisode['season'].$tunefindadd.'" target="_blank" onclick="document.getElementById(\'songs\').focus();" >TuneFind</a>';
	//echo '</span><br><br>';
...
<script type="text/javascript">
	...
	var disqus_shortname = 'allmusiclyricsinfo'; // required: replace example with your forum shortname
