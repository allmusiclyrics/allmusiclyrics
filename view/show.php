...
if(!isset($getShowID)){
	if(isset($_GET['q']))$getShowID = getShowID($_GET['q'],1);
	else $getShowID = getShow($_GET['showid'],1);
}
...
if($getShowID['lastseason']){
	for ($i = 1; $i <= $getShowID['lastseason']; $i++) {
		...
		foreach($getEpisode as $getEpisode){
			if($_SESSION){
				if($getEpisode['season']==$i)include('list.php');
			}else{
				if($getEpisode['season']==$i&&$getEpisode['timestamp']<time())include('list.php');
			}
		}
	}
}